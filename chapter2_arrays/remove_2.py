...
class IntArray():
    ...
    def remove(self, index: int) -> int:
        """
    Remove the element at the given index and return its value.
    ...
    # Decrease the array size
        ...
        if self._size > 0:
        # Create new reserved memory for the smaller array
            new_resmem = ReservedMemory(self._size * self._bytes_per_element)
        ...
        # Copy elements after the removed index
            bytes_after = (self._size - index) * self._bytes_per_element
            if bytes_after > 0:
                new_resmem.copy(
                    self._resmem,
                    count=bytes_after,
            ...
            new_resmem = None  # Array is now empty
