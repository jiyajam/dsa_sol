...
class HashTable():
    ...
    def put(self, key, value):
        ...
        # First, try to find the key (to update it if it exists)
        key_index = self._find_key(start, key)
        if key_index is not None:
            self.slots[key_index].value = value
            return

        # If key is not found, check if table is full
        if self.used_slots == self.size:
            raise MemoryError("HashTable is full")

        # Find a free slot to insert the new key
        free_index = self._find_free_slot(start)
